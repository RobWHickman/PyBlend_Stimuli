...
#################################################
#	CONTROL HOW RANDOM THE GENERATED TREE IS	#
#################################################
...
Basic_debugging_tree = False

#############################################
#	CONTROL THE ENVIRONMENT OF THE TREE		#
#############################################
#What should the environment look like?
#colour the tree/leaves randomly
Colour_random = False
#transparent background or sky
...
#############################################
#	CONTROL THE OUTPUT OF GENERATED TREES	#
#############################################
...
#adds a tree (or cube) 
def tree_add(x, y, var1_mu, var1_sig, var2_mu, var2_sig):
	if Tree_not_cube:
...
def render_scene(output_folder)
	if SysTime_save:
	...
	if Controlled_randomness:
		for x_repeat in range(Grid_size):
			for y_repeat in range(Grid_size):
				...
				grid_subdirectory = path.join(run_directory, (str(x_repeat) + str(y_repeat)))
				if not path.exists(grid_subdirectory):
					makedirs(grid_subdirectory)
				#adds trees and cameras and environment
				tree_add(x = x_repeat, y = y_repeat, var1_sig = 5, var1_mu = 15, var2_sig = 3, var2_mu = 1)
				production_add(obj_position = obj_centre, camera_radius = 22)
				environment_add()
				#render it all
				render_scene(output_folder = path.join(grid_subdirectory))
	else:
		#adds trees and cameras and environment
		...
		environment_add()
		#render it all
		render_scene(output_folder = run_directory)
