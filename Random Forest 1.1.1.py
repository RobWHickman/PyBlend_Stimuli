...
def render_scene(output_folder, iteration):
	if SysTime_save:
		image_string = datetime.now().strftime("%d-%m:::%H:%M:%S")
	else:
		image_string = ("iteration: " + str(iteration))
	...
	if Controlled_randomness:
		for x_repeat in range(Grid_size):
			for y_repeat in range(Grid_size):
				...
				environment_add()
				#render it all
				render_scene(output_folder = path.join(grid_subdirectory), iteration = repeats)
	else:
		#adds trees and cameras and environment
