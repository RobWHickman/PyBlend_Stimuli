...
import os.path
import bpy
from math import sqrt, radians, pi, cos, sin, acos, fabs
...
#make a tree/cube
Tree = True
#make a truly random tree or a basic one for debugging
Random_Forest = True
#make a semi-random tree that varies only along leaves/branches parameter
BranchvsLeaf = False #**WIP**
...
def tree_plotter(save_folder):
	...
	if Tree:
		...
		#or a basic one for debugging
			...
			obj = bpy.ops.curve.tree_add(bevel = True, baseSize = 0.1, baseSplits = 2, levels = 3,
				showLeaves = True, leaves = 50)
			obj_centre = bpy.data.objects['tree'].location
