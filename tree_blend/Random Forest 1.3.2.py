...
Output_directory = "/home/robert/Desktop/Summerfield Lab/My Code/Tree Scripts/Generated Trees/imagebin"
...
def tree_add(x, y, var1_mu, var1_sig, var2_mu, var2_sig, iteration,
	Tree_not_cube = True, Controlled_randomness = True, Basic_debugging_tree = True):
	...
	if Tree_not_cube:
		...
		if Controlled_randomness:
			...
			if Basic_debugging_tree: #basic set tree
				obj = bpy.ops.curve.tree_add(bevel=True,showLeaves=True,scale=11,baseSize=0.1,baseSplits=2,
					segSplits=(0.24,0.24,0,0),splitAngle=(6.475,6.475,6.475,6.475),attractUp=0.28,levels=4,
					length=(1.027,0.327,0.627,0.477),branches=(56,36,16,16),leaves=20,leafScale=0.1,leafScaleX=1.65)
			else: #to form grids of trees
				obj = bpy.ops.curve.tree_add(bevel=True,
							showLeaves=True,
							seed = iteration,
							scale=randomgen.randint(11,13),
							baseSize=0.1,
							baseSplits=3,
							segSplits=(0.24,0.24,0,0),
							splitAngle=(randomgen.gauss(6.5, 5),randomgen.gauss(6.5, 5),randomgen.gauss(6.5, 5),randomgen.gauss(6.5, 5)),
							attractUp=randomgen.gauss(0.3,0.1),
							levels=4,
							length=(1.027,0.327,0.627,0.477),
							leafScale=0.08,
							leafScaleX=randomgen.gauss(0.15, 0.05),
...
def environment_add(Texture_folder, Textured = True, Colour_random = True, Transparent_background = True):
	...
	if Textured:
		...
		tex_number = randomgen.randint(1,16)
		leaf_file = path.join(Texture_folder, ("leaf" + str(tex_number) + ".jpg"))
	...
	if Textured:
		...
		tex_number = randomgen.randint(1,16)
		bark_file = path.join(Texture_folder, ("treebark" + str(tex_number) + ".jpg"))
		mat = bpy.data.materials.new("material")
		mat_texture = mat.texture_slots.add()
	...
	if Grid_size > 1:
		for x_repeat in range(Grid_size):
			for y_repeat in range(Grid_size):
				...
				environment_add(Texture_folder = "/home/robert/Desktop/Summerfield Lab/My Code/Tree Scripts/Tree Textures")
				#render it all
		...
		environment_add(Texture_folder = "/home/robert/Desktop/Summerfield Lab/My Code/Tree Scripts/Tree Textures")
		#render it all
		render_scene(output_folder = run_directory, grid_pos = "tree", iteration = repeats)
